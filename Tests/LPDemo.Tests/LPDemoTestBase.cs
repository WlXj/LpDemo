...
namespace LPDemo.Tests
{
    public abstract class LPDemoTestBase : AbpIntegratedTestBase<LPDemoTestModule>
    {
        ...
        protected void UsingDbContext(Action<LPDemoDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<LPDemoDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<LPDemoDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<LPDemoDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<LPDemoDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<LPDemoDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<LPDemoDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<LPDemoDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<LPDemoDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<LPDemoDbContext, Task<T>> func)
        {
            T result;
